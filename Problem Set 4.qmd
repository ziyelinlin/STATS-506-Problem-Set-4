...
tennis_collapsed <- tennis %>%
  mutate(tourney_key = if_else(tourney_level == "D", "2019-M-DC-2019", tourney_id)) 
...
Yes, 12 players won more than one tournament; the most winningplayers (Novak Djokovic and Dominic Thiem) won was 5 tournaments.
...
Same as previous part, states began reporting on different dates, so I restricted the time axis to dates when all 50 states were reporting and excluded D.C. and five major territories. For each state I computed a the **mean** of `cases_avg_per_100k` over the study window to identity the states with the highest and lowest overall rates per population. When plotting, I overlaid the national median (across states, same exclusions) as a reference line in each panel.

```{r}
valid_starting_date <- covid %>% 
  filter(!(state %in% c("American Samoa", 
                        "Guam", 
                        "Northern Mariana Islands", 
                        "Puerto Rico", 
...
state_overall <- covid %>% 
  filter(!(state %in% c("American Samoa", 
                        ...
                        "Virgin Islands",
                        "District of Columbia")),
         date >= valid_starting_date) %>% 
  group_by(state) %>% 
  summarize(overall_per_100k = mean(cases_avg_per_100k, na.rm = TRUE),
            .groups = "drop") %>% 
  arrange(desc(overall_per_100k))
...
ggplot(traj, aes(date, cases_avg_per_100k)) +
  ...
  labs(title = "Covid-19 Trajectories for Highest vs Lowest Overall Rates per 100k",
...
Using the stated rules, the state with the highest overall rate is Rhode Island and the lowest is Maine.
...
I defined “substantial” as the first date a state reached ≥ 2 deaths per 100k using `deaths_avg_per_100k`, focusing on 50 U.S. states only. For each state I computed the earliest date meeting the threshold, then sorted by that date and took the first five states. In the plots, a national median reference line (median over states each day) is included for visual comparison.
...
earliest_5_states <- earliest5 %>% pull(state)

plot_df <- covid %>% filter(state %in% earliest_5_states)

nation_ref <- covid %>%
  filter(!(state %in% c("American Samoa","Guam","Northern Mariana Islands",
                        "Puerto Rico","Virgin Islands","District of Columbia"))) %>%
  group_by(date) %>%
  summarise(us_median = median(deaths_avg_per_100k, na.rm = TRUE), .groups = "drop")
```

```{r fig.width=9, fig.height=12, fig.retina=2}
covid %>% 
  filter(state %in% earliest_5_states) %>% 
  ggplot(., aes(x = date, y = deaths_avg_per_100k)) +
  geom_line(color = "black", linewidth = 0.5) +
  geom_line(data = nation_ref, 
            aes(y = us_median), 
            color = "grey") +
  geom_vline(data = earliest5,
             aes(xintercept = first_date), 
             linetype = "dashed",
             linewidth = 0.2,
             color = "brown") + 
  geom_text(data = earliest5,
            aes(x = first_date, 
                y = Inf,
                label = paste0(format(first_date, "%b %d, %Y"))),
            color = "brown", 
            size = 3) +
  facet_wrap(~ state, ncol = 1) +
  coord_cartesian(clip = "off") +
  scale_y_continuous("7-day deaths per 100k", 
                     labels = label_number(accuracy = 0.1)) +
  labs(title = "Earliest States to Experience Substantial Covid Mortality",
       subtitle = "dashed line marks first date with ≥ 2 deaths per 100k, grey line is the national median",
       x = NULL) +
  theme_minimal() +
...
The dashed line in each panel marks the first day the state’s 7-day death rate reached ≥ 2 per 100k. Based on the plot, the first five states to experience Covid substantially were New York (Apr 03, 2020), New Jersey (Apr 10, 2020), Connecticut (Apr 15, 2020), Massachusetts (Apr 20, 2020), and North Dakota (Nov 08, 2020). Interestingly, the top four are tightly clustered in the Northeast during the initial spring 2020 wave.
